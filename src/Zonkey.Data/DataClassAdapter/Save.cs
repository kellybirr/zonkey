...
namespace Zonkey
{
	public partial class DataClassAdapter<T>
	{
		...
		{
			return HandleSaveResult( await TrySave(obj, UpdateCriteria.Default, UpdateAffect.ChangedFields, SelectBack.Default) );
		}
		...
		{
			return HandleSaveResult( await TrySave(obj, criteria, UpdateAffect.ChangedFields, SelectBack.Default) );
		}
		...
		{
			return HandleSaveResult( await TrySave(obj, criteria, affect, selectBack) );
		}
		...
		public async Task<SaveResult> TrySave(T obj, UpdateCriteria criteria, UpdateAffect affect, SelectBack selectBack)
		{
			...
			if (objSV == null) throw new ArgumentException("Save() is only supported on classes that implement Zonkey.ObjectModel.ISavable", nameof(obj));
			...
			switch (objSV.DataRowState)
			{
				case DataRowState.Added:
					result = await TryInsert(obj, selectBack);
					break;
				...
				case DataRowState.Modified:
					if (((selectBack == SelectBack.None) || (selectBack == SelectBack.AllFields))
						...
						result = await TryUpdate2(obj, criteria, (selectBack == SelectBack.AllFields));
					else
						result = await TryUpdate(obj, criteria, affect, selectBack);
		...
		{
			var result = await TrySaveCollection(collection, criteria, affect, selectBack);
			...
			foreach (T obj in collection)
			{
				...
				if (objSV != null)
				{
					if (objSV.DataRowState != DataRowState.Unchanged)
					{
						try
						{
							SaveResult saveResult = await TrySave(obj, criteria, affect, selectBack);
		...
		public async Task<SaveResult> TryUpdate(T obj, UpdateCriteria criteria, UpdateAffect affect, SelectBack selectBack)
		{
			try
			{
				...
				return await UpdateInternal(obj, commands);
			}
			catch (Exception ex)
			{
		...
		{
			return HandleSaveResult( await TryUpdate(obj, criteria, affect, selectBack) );
		}
		...
		{
			return HandleSaveResult( await TryUpdate2(obj, criteria, doSelectBack) );
		}
		...
		private async Task<SaveResult> UpdateInternal(T obj, DbCommand[] commands)
		{
			...
			// execute update;
			int nRecordsAffected = await ExecuteNonQueryInternal(updateCommand);
			...
			using (DbDataReader reader = await ExecuteReaderInternal(readerCommand, CommandBehavior.SingleRow))
			{
		...
		public async Task<SaveResult> TryInsert(T obj, SelectBack selectBack)
		{
			try
			{
				...
				{
					int nRecordsAffected = await ExecuteNonQueryInternal(commands[0]);
		...
		{
			return HandleSaveResult( await TryInsert(obj, selectBack) );
		}
