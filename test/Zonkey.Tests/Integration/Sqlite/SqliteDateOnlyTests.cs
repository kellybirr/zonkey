...
namespace Zonkey.Tests.Integration.Sqlite
{
    ...
    public class SqliteDateOnlyTests : IClassFixture<SqliteFixture>
    {
        ...
        public SqliteDateOnlyTests(SqliteFixture db) => _db = db;
        ...
        private static async Task EnsureTable(DbConnection conn)
        {
            ...
            cmd.CommandText = "CREATE TABLE IF NOT EXISTS date_probe (id INTEGER PRIMARY KEY AUTOINCREMENT, d date, t time)";
        ...
        [Fact]
        public async Task Text_ReadsStoredText_BothModes()
        {
            using var conn = _db.CreateConnection();
            await EnsureTable(conn);
