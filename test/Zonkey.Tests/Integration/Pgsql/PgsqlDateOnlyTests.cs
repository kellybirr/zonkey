...
{
    /// <summary>
    /// PostgreSQL date/time column round-trips across provider majors. On the net8.0 leg
    /// (Npgsql 9) the provider surfaces date/time as DateTime/TimeSpan; on the net10.0 leg
    /// (Npgsql 10) it surfaces DateOnly/TimeOnly -- a documented Npgsql 10 breaking change.
    /// Zonkey's conversions are symmetric, so every entity shape below must round-trip
    /// identically on both legs and both materialization paths.
    /// </summary>
    public class PgsqlDateOnlyTests : IClassFixture<PgsqlFixture>
    {
        private readonly PgsqlFixture _db;

        public PgsqlDateOnlyTests(PgsqlFixture db) => _db = db;
        ...
        private static async Task EnsureTable(DbConnection conn)
        {
            ...
            cmd.CommandText = "CREATE TABLE IF NOT EXISTS date_probe (id SERIAL PRIMARY KEY, d date, t time)";
